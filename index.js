...
app.get("/mes_bitcoins", async (req, res) => {
  mes_bitcoins = await connection("SELECT * FROM `mes_bitcoins` ORDER BY `mes_bitcoins`.`date` DESC LIMIT 24")
...
app.get("/mes_euros", async (req, res) => {
  mes_euros = await connection("SELECT * FROM `mes_euros` ORDER BY `mes_euros`.`date` DESC LIMIT 24")
...
app.get("/bitcoins_cours", async (req, res) => {
  bitcoins_cours = await connection("SELECT * FROM `bitcoins_cours` ORDER BY `bitcoins_cours`.`date` DESC LIMIT 24")
...
app.post("/mes_bitcoins", jsonParser, async function (req, res) {
  ...
  mes_bitcoins = await connection("SELECT * FROM `mes_bitcoins` ORDER BY `mes_bitcoins`.`date` DESC LIMIT 1")
  mes_bitcoins = Object.values(JSON.parse(JSON.stringify(mes_bitcoins)))
  nouvelle_quantite_bitcoins = mes_bitcoins[0].quantite - quantite_bitcoins_demande_a_vendre
  console.log("🚀 ~ file: index.js ~ line 78 ~ nouvelle_quantite_bitcoins", nouvelle_quantite_bitcoins)

  bitcoins_cours = await connection("SELECT * FROM `bitcoins_cours` ORDER BY `bitcoins_cours`.`date` DESC LIMIT 1")
  bitcoins_cours = Object.values(JSON.parse(JSON.stringify(bitcoins_cours)))
  bitcoins_cours = bitcoins_cours[0].valeur
  console.log("🚀 ~ file: index.js ~ line 83 ~ bitcoins_cours", bitcoins_cours)

  ancienn_quantite_euro = await connection("SELECT * FROM `mes_euros` ORDER BY `mes_euros`.`date` DESC LIMIT 1")
  ancienn_quantite_euro = Object.values(JSON.parse(JSON.stringify(ancienn_quantite_euro)))[0].quantite
  nouvelle_quantite_euros = ancienn_quantite_euro + convert_bitcoins_to_euros(quantite_bitcoins_demande_a_vendre, bitcoins_cours)
  console.log("🚀 ~ file: index.js ~ line 86 ~ nouvelle_quantite_euros", nouvelle_quantite_euros)

  console.log("🚀 ~ file: index.js ~ line 88 ~ req.body.date", req.body.date)
  db.query("INSERT INTO `mes_euros` (`id`, `monnaie`, `quantite`, `date`) VALUES (NULL, '" + "euros" + "', '" + nouvelle_quantite_euros + "', '" + req.body.date + "')")
  db.query("INSERT INTO `mes_bitcoins` (`id`, `monnaie`, `quantite`, `date`) VALUES (NULL, '" + "bitcoins" + "', '" + nouvelle_quantite_bitcoins + "', '" + req.body.date + "')")
...
app.post("/mes_euros", jsonParser, async function (req, res) {
  ...
  mes_euros = await connection("SELECT * FROM `mes_euros` ORDER BY `mes_euros`.`date` DESC LIMIT 1")
  mes_euros = Object.values(JSON.parse(JSON.stringify(mes_euros)))
  nouvelle_quantite_euros = mes_euros[0].quantite - quantite_euros_demande_a_vendre
  console.log("🚀 ~ file: index.js ~ line 96 ~ nouvelle_quantite_euros", nouvelle_quantite_euros)

  bitcoins_cours = await connection("SELECT * FROM `bitcoins_cours` ORDER BY `bitcoins_cours`.`date` DESC LIMIT 1")
  bitcoins_cours = Object.values(JSON.parse(JSON.stringify(bitcoins_cours)))
  bitcoins_cours = bitcoins_cours[0].valeur
  console.log("🚀 ~ file: index.js ~ line 101 ~ bitcoins_cours", bitcoins_cours)

  ancienne_qantite_bitcoins = await connection("SELECT * FROM `mes_bitcoins` ORDER BY `mes_bitcoins`.`date` DESC LIMIT 1")
  ancienne_qantite_bitcoins = Object.values(JSON.parse(JSON.stringify(ancienne_qantite_bitcoins)))[0].quantite

  nouvelle_quantite_bitcoins = ancienne_qantite_bitcoins + convert_euros_to_bitcoins(quantite_euros_demande_a_vendre, bitcoins_cours)
  console.log("🚀 ~ file: index.js ~ line 103 ~ nouvelle_quantite_bitcoins", nouvelle_quantite_bitcoins)
...
db.promise = (sql) => {
  ...
  console.log("🚀 ~ file: index.js ~ line 129 ~ sql", sql)
...
const save_all_in_db = async function () {
  date_now = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 19).replace("T", " ")
